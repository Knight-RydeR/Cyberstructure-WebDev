...
const BestPrice = () => {
    ...
    useEffect(() => {
    authAxiosDefault.get(`/communityBuilds`).then(response => {
        ...
        return (
            <div>
                <Nav name="Best Price" />
                {selectedItem && <Popup type="bestPrice" trigger={buttonPopup} setTrigger={setButtonPopup} setCard={setCardInvis} /*category={selectedItem.category} name={selectedItem.nameOfProduct} price={selectedItem.price}*/ CPU={selectedItem.Processor} MB={selectedItem.Motherboard} RAM={selectedItem.Ram} PSU={selectedItem.PSU} HD={selectedItem.Storage} GPU={selectedItem.GPU} COOL={selectedItem.Cooler} FANS={selectedItem.Fans} CS={selectedItem.Case} rate={selectedItem.buildRating}>
                </Popup>}
